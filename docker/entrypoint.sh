...
EXTRA_DDL = [
    ...
    # Tutor portal requests — keep self-service request review table available on older deployments
    ...
    "CREATE TABLE IF NOT EXISTS deleted_attendance_sessions (\n        id                  SERIAL PRIMARY KEY,\n        original_session_id INTEGER NOT NULL,\n        restored_session_id INTEGER REFERENCES attendance_sessions(id),\n        payload_json        JSONB NOT NULL DEFAULT '{}',\n        deleted_by          INTEGER REFERENCES users(id),\n        deleted_at          TIMESTAMP DEFAULT NOW() NOT NULL,\n        restored_by         INTEGER REFERENCES users(id),\n        restored_at         TIMESTAMP\n    )",
    "CREATE INDEX IF NOT EXISTS ix_deleted_attendance_sessions_original_session_id ON deleted_attendance_sessions(original_session_id)",
    "CREATE INDEX IF NOT EXISTS ix_deleted_attendance_sessions_restored_session_id ON deleted_attendance_sessions(restored_session_id)",
